    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto transition-all duration-300 relative">
        ...
        <div class="px-8 pt-2 pb-10">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ url('/tambah-kpa') }}" class="px-6 py-2 border border-[#2a93c9] text-[#2a93c9] text-[13px] font-medium rounded hover:bg-blue-50 transition-all duration-300 tracking-wide bg-white shadow-sm">
                    TAMBAH
                </a>
            </div>
            ...
            <div class="bg-white rounded border border-[#e2e8f0] shadow-sm overflow-hidden min-h-[500px]">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody>
                        @forelse($dataKpa as $index => $item)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                <td class="py-4 px-6 text-[13px] text-center text-[#4a9bc8] border-r border-gray-100">{{ $index + 1 }}</td>
                                <td class="py-4 px-6 text-[13px] text-[#4a9bc8] border-r border-gray-100">{{ $item->tahun_anggaran }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-700 font-medium border-r border-gray-100">{{ $item->nama_kpa }}</td>
                                <td class="py-4 px-6 text-[13px] text-[#4a9bc8] border-r border-gray-100">{{ $item->nip_kpa ?? '-' }}</td>
                                <td class="py-4 px-6 text-[13px] text-[#4a9bc8] border-r border-gray-100 font-mono">{{ $item->nomor_dipa }}</td>
                                <td class="py-4 px-6 text-[13px] text-[#2ebd59] font-medium border-r border-gray-100">
                                    {{ $item->tanggal_dipa ? \Carbon\Carbon::parse($item->tanggal_dipa)->format('d-m-Y') : '-' }}
                                </td>
                                <td class="py-4 px-6 text-center whitespace-nowrap">
                                    <div class="flex justify-center gap-3 text-lg">
                                        <a href="{{ url('/edit-kpa/' . $item->id) }}" class="text-[#2a93c9] hover:opacity-70 transition" title="Edit">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        <form action="{{ url('/hapus-kpa/' . $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data KPA/DIPA ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-10 text-center text-gray-400 italic text-[13px]">Belum ada data KPA/DIPA.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
