...
<head>
    ...
    <title>Data Pegawai dan Mitra Statistik - MODUS BPS</title>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto transition-all duration-300 relative">
        ...
        <div class="px-8 pt-2 pb-10">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ url('/tambah-pegawai') }}" class="px-6 py-2 border border-[#2a93c9] text-[#2a93c9] text-[13px] font-medium rounded hover:bg-blue-50 transition-all duration-300 tracking-wide bg-white shadow-sm">
                    TAMBAH
                </a>
            </div>
                    ...
                    <tbody>
                        @forelse($pegawai as $index => $item)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                <td class="py-4 px-6 text-[13px] text-[#4a9bc8] border-r border-gray-100">{{ $index + 1 }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-700 font-medium border-r border-gray-100 text-left">{{ $item->nama }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-600 border-r border-gray-100">{{ $item->status_pegawai }}</td>
                                <td class="py-4 px-6 text-[13px] text-[#4a9bc8] border-r border-gray-100 font-mono">{{ $item->nip }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-600 border-r border-gray-100 text-left">{{ $item->alamat ?? '-' }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-600 border-r border-gray-100">{{ $item->no_telepon ?? '-' }}</td>
                                <td class="py-4 px-6 text-center whitespace-nowrap">
                                    <div class="flex justify-center gap-3 text-lg">
                                        <a href="{{ url('/edit-pegawai/' . $item->id) }}" class="text-[#2a93c9] hover:opacity-70 transition" title="Edit">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        <form action="{{ url('/hapus-pegawai/' . $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data pegawai/mitra ini?');">
                                            @csrf
                                            @method('DELETE')
