...
<head>
    ...
    <title>Tambah Kegiatan Teknis - MODUS BPS</title>
...
<body class="flex h-screen overflow-hidden text-gray-800">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto">
        <div class="px-8 pt-10 pb-10 flex flex-col">
            <h1 class="text-[22px] font-semibold text-black tracking-tight mb-8">Tambah Kegiatan Teknis</h1>
            
            <div class="flex justify-center mt-4">
                <div class="bg-white border border-[#e2e8f0] shadow-sm p-10 rounded-sm w-[500px]">
                    <form action="{{ url('/tambah-kegiatan-teknis') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" name="nama_teknis" value="{{ old('nama_teknis') }}" class="form-input-custom" placeholder="Misal: SUSENAS" required>
                        </div>

                        <div class="mb-5">
                            <label class="block text-[#4a9bc8] text-[13px] mb-2 font-medium">Nama Survei / Kegiatan</label>
                            <input type="text" name="nama_survei" value="{{ old('nama_survei') }}" class="form-input-custom" placeholder="Misal: Survei Sosial Ekonomi Nasional" required>
                        </div>

                        <div class="mb-8">
                            <label class="block text-[#4a9bc8] text-[13px] mb-2 font-medium">Periode / Tahun</label>
                            <input type="text" name="periode" value="{{ old('periode') }}" class="form-input-custom" placeholder="Contoh: 2026" required>
                        </div>
