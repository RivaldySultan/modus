    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto transition-all duration-300 relative">
        ...
        <div class="px-8 pt-2 pb-10">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ url('/tambah-teknis') }}" class="px-6 py-2 border border-[#2a93c9] text-[#2a93c9] text-[13px] font-medium rounded hover:bg-[#2a93c9] hover:text-white transition-all duration-300 tracking-wide bg-white shadow-sm">
                    TAMBAH
                </a>
            </div>
            ...
            <div class="bg-white rounded border border-[#e2e8f0] shadow-sm overflow-hidden min-h-[500px]">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-gray-700">
                        @forelse($dataTeknis as $index => $item)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                <td class="py-4 px-6 text-[13px] text-center text-[#2a93c9] font-medium">{{ $index + 1 }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-700">{{ $item->keterangan }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-700">{{ $item->nama_teknis }}</td>
                                <td class="py-4 px-6 text-[13px] text-gray-700">{{ $item->kode_teknis ?? '-' }}</td>
                                <td class="py-4 px-6 text-center whitespace-nowrap">
                                    <a href="{{ url('/edit-teknis/' . $item->id) }}" class="text-gray-700 hover:text-[#2a93c9] mr-3 transition" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>
                                    <form action="{{ url('/hapus-teknis/' . $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-gray-700 hover:text-red-600 transition" title="Hapus">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-10 text-center text-gray-400 italic text-[13px]">Belum ada data teknis.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
