    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto transition-all duration-300 relative">
        ...
        <div class="px-8 pt-2 pb-10 flex flex-col">
            ...
            <div class="flex justify-center w-full">
                <div class="bg-white border border-[#e2e8f0] shadow-sm p-8 rounded-sm w-[450px]">
                    ...
                    <form id="formEditTemplate">
                        ...
                        <div class="mb-5 relative">
                            ...
                            <div id="customSelectKelompok" class="w-full border border-[#4a9bc8] rounded h-[40px] px-3 flex items-center justify-between bg-white cursor-pointer select-none">
                                <span id="selectTextKelompok" class="text-[#4a9bc8] text-[14px]"></span> 
                        ...
                        <div class="mb-2">
                            <label class="block text-[#4a9bc8] text-[14px] mb-2 font-medium">Upload Dokumen Baru <span class="text-[11px] text-gray-400 font-normal">(Abaikan jika tidak diubah)</span></label>
                            <div class="w-full border border-[#4a9bc8] rounded h-[120px] bg-white cursor-pointer hover:bg-blue-50/30 transition relative overflow-hidden">
                                <div class="absolute inset-0 flex items-center justify-center p-4 z-0 pointer-events-none">
                                    <span id="nama_file_tampil" class="text-[13px] text-[#4a9bc8] font-medium text-center truncate w-full">Memuat...</span>
                                </div>
                                
                                <input type="file" id="file_dokumen" accept=".doc,.docx" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                            </div>
    ...
    <script>
        ...
        fileInput.addEventListener('change', function() {
            if(this.files && this.files.length > 0) {
                namaFileTampil.textContent = "File baru: " + this.files[0].name;
            }
        });


        // --- SCRIPT AMBIL DATA LAMA UNTUK DIEDIT ---
        ...
        if (editIndex !== null && templates[editIndex]) {
            let dataToEdit = templates[editIndex];
            
            // Set Kelompok SK (Sebelumnya disimpan di property 'nama')
            const kelompokVal = dataToEdit.nama;
            hiddenInputKelompok.value = kelompokVal;
            selectTextKelompok.textContent = kelompokVal;
            
            // Render Jenis SK list sesuai Kelompok SK
            renderJenisSK(kelompokVal);
            
            // Set Jenis SK
            const jenisVal = dataToEdit.jenis;
            hiddenInputJenis.value = jenisVal;
            selectTextJenis.textContent = jenisVal;
            
            // Tampilkan nama file yang tersimpan
            namaFileTampil.textContent = "File tersimpan: " + (dataToEdit.file || "Tidak ada file");
        } else {
        ...
        document.getElementById('formEditTemplate').addEventListener('submit', function(e) {
            ...
            const kelompokSk = hiddenInputKelompok.value;
            const jenisSk = hiddenInputJenis.value;
            
            if(!kelompokSk || !jenisSk) {
                alert('Silakan lengkapi Kelompok SK dan Jenis SK!');
                return;
            }

            if (editIndex !== null && templates[editIndex]) {
                // Update Kelompok dan Jenis
                templates[editIndex].nama = kelompokSk; 
                templates[editIndex].jenis = jenisSk;
                
                // Jika user mengupload file baru, timpa file lama
                if(fileInput.files.length > 0) {
                    templates[editIndex].file = fileInput.files[0].name;
                }
                
                // Simpan dan arahkan kembali
